...
class Interaction(commands.Cog):
    ...
    @commands.slash_command(name="8ball", guild_ids=[guild])
    async def _8ball(self, ctx, question):

        embed = discord.Embed(
            title=question, description=random.choice(ball_responses))

        await ctx.respond(embed=embed)

		@commands.slash_command(name="clear", guild_ids=[guild])
		async def _clear(self, ctx, amount):
			 await ctx.channel.purge(amount)
			
    @commands.slash_command(name="confess", guild_ids=[guild])
    async def confess(self, ctx, confession):

        embed = discord.Embed(
            title='Confession', description=confession, color=discord.Colour.random())
