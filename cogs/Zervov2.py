...
class Zervo(commands.Cog):
    ...
    async def _usersearch(self, ctx, username):
        ...
        if response['code'] != 200:
            response = requests.get(f"https://wg6.pinpon.cool/pinpon-app-system/v5/app-recommend/all/search?current=1&name={username}&size=3",headers=self.bot.headers).json()
    ...
    async def _register(self,ctx ,username):
        ...
        count = 1
        user = c.db.execute(
                f'select * from users where dId =?', (ctx.author.id,)).fetchone()
        print(user)
        if user != None:
        ...
        user = response['data']
        if response['code'] == 200:
            c.db.execute("insert into users values(?,?,?,?,'Null',1)",(ctx.author.id,user['appUserId'],username,str(ctx.author.avatar)))
            c.conn.commit()
