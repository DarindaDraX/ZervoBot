...
class MyView(discord.ui.View):
    ...
    def __init__(self, bot, ctx, keyword, if_mention, if_none, *args,
                 **kwargs) -> None:
    ...
    async def button_callback2(self, button, interaction):
        ...
        c.db.execute(
            "insert into reactions (dId,keyword,desc,link,if_mention,if_None) values(?,?,?,?,?,?)",
            (interaction.user.id, self.keyword, data[3], data[2],
             self.if_mention, self.if_None))
        c.conn.commit()
        embed = discord.Embed(
            description=
            f"Press **Yes** if image match with keyword **{self.keyword}**\n\n Else **No** \n**Do not Wrong vote**\n\n"
        )
...
class Custom(commands.Cog):
    ...
    async def _add(self, ctx, query, keyword, if_mention, if_none):
        ...
        if "author" and "mention" not in if_mention:
            await ctx.respond(
                "**if_mention** must contain word 'author' and 'mentiion'\n**Example** author kissing to mention\n"
            )
            return
        if "author" not in if_none:
            await ctx.respond(
                "**if_none** must contain word 'author'\n**Example** author kissing\n"
            )
            return
        data = []
        global db
        ...
        if data:
            ...
            embed = discord.Embed(
                title='Add keyword',
                description=
                f'Press **Yes** if image match with keyword **{keyword}**\n\n Else **No** \n**Do not Wrong vote**\n\n'
            )
    ...
    async def on_message(self, message):
        ...
        if message.content.lower().startswith("zay"):
            ...
            cmd = msg[0]
            keywords = []
            lkeywords = self.bot.db.execute(
                'select DISTINCT keyword from reactions').fetchall()
            for x in lkeywords:
                keywords.append(x[0])
            if cmd in keywords:
                data = self.bot.db.execute(
                    "select * from reactions where keyword = ?",
                    (cmd, )).fetchall()
                ...
                if user == None:
                    ...
                    mes = img[5]
                    print(mes)
                    text = mes.replace('author', message.author.name).replace(
                        'mention', user)
                    embed = discord.Embed(description=text)
